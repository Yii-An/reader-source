...
export const universalRuleSchema = {
  ...
  properties: {
    ...
    contentType: {
      type: 'string',
      description:
        '内容类型: novel(小说) | manga(漫画) | video(视频) | audio(音频) | rss | novelmore',
  ...
  $defs: {
    searchRule: {
      ...
      properties: {
        ...
        url: {
          type: 'string',
          description: '搜索 URL 模板，使用 $keyword 表示搜索词，$page 表示页码'
        },
        list: {
          type: 'string',
          description: '搜索结果列表选择器 (支持 @css: / @xpath: / @json: / @js: 前缀)'
        },
        name: {
          type: 'string',
        ...
        anyReader: {
          ...
          properties: {
            items: { type: 'string', description: '搜索结果项模板' }
          }
        }
      }
    },
    ...
    chapterRule: {
      ...
      properties: {
        url: { type: 'string', description: '章节列表 URL (使用 $result 引用上一步结果)' },
        ...
        result: { type: 'string', description: '章节 URL 选择器 (正文地址)' },
        ...
        anyReader: {
          type: 'object',
          properties: {
            items: { type: 'string', description: '章节项模板' },
            lock: { type: 'string', description: '章节锁定标识选择器' }
          }
        }
      }
    },
    discoverRule: {
      ...
      properties: {
        ...
        url: { type: 'string', description: '发现页 URL 或分类配置 (支持 @js: 动态生成)' },
        ...
        anyReader: {
          type: 'object',
          properties: {
            items: { type: 'string', description: '发现结果项模板' }
          }
        }
      }
    },
    contentRule: {
      ...
      properties: {
        ...
        legado: {
          type: 'object',
          properties: {
            webJs: { type: 'string', description: 'WebView 执行的 JS 脚本' },
            replaceRegex: { type: 'string', description: '正则替换规则' },
            imageStyle: { type: 'string', description: '图片样式配置' }
          }
        }
      }
    },
    anyReaderBaseFields: {
      ...
      properties: {
        useCryptoJS: { type: 'boolean', description: '是否使用 CryptoJS 加密库' },
        cookies: { type: 'string', description: 'Cookie 存储' },
        postScript: { type: 'string', description: '脚本后处理' },
        viewStyle: { type: 'integer', description: '视图样式 (0: 默认)' }
      }
    },
    legadoBaseFields: {
      type: 'object',
      ...
      properties: {
        bookUrlPattern: { type: 'string', description: '书籍 URL 正则匹配' },
        enabledCookieJar: { type: 'boolean', description: '是否启用 Cookie 罐' },
        respondTime: { type: 'integer', description: '响应超时时间 (毫秒)' },
        weight: { type: 'integer', description: '权重 (用于排序)' },
        customOrder: { type: 'integer', description: '自定义排序' },
        lastUpdateTime: { type: 'integer', description: '最后更新时间 (时间戳)' },
        loginUrl: { type: 'string', description: '登录 URL' },
        loginCheckUrl: { type: 'string', description: '登录检测 URL' },
        loginUi: { type: 'string', description: '登录 UI 配置' },
        loginCheckJs: { type: 'string', description: '登录检测 JS 脚本' }
      }
    },
    ruleMeta: {
      type: 'object',
